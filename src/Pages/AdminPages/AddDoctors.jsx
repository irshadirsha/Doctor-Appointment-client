...
function AddDoctors() {
    ...
    const onSubmitHandler = async (event) => {
        ...
        if (!docImg) {
          ...
          // Step 1: Create formData for Cloudinary
        const cloudinaryFormData = new FormData();
        cloudinaryFormData.append('file', docImg); // The image file
        cloudinaryFormData.append('upload_preset', 'ml_default'); // Preset for Cloudinary uploads (you can customize)
        // const  data = await axios.post(`https://api.cloudinary.com/v1_1/${cloudName}/image/upload?upload_preset=I-club`,formData);
        try {
            // Step 2: Upload to Cloudinary
            const cloudinaryRes = await axios.post(`https://api.cloudinary.com/v1_1/${cloudName}/image/upload?upload_preset=I-club`,
                cloudinaryFormData
            );

            const secureUrl = cloudinaryRes.data.secure_url; // Step 3: Get secure URL from response
            console.log('Image URL:', secureUrl);
    
         // Step 4: Prepare the rest of the form data
         const formData = {
            name,
            email,
            password,
            ...
            image: secureUrl // Step 5: Add secure URL to form data
        };
       ...
       console.log('response----',response.data.status)
        ...
        console.error('Error uploading to Cloudinary or adding doctor:', error);
        toast.error('Error uploading image or adding doctor');
    }
};
const resetForm = () => {
    setName('');
    ...
    setAddress1('');
    setAddress2('');
    setDocImg(false); // Reset the image
};
...
//         // API call is removed, but the structure remains for future implementation
